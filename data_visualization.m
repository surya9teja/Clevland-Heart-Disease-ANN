...
col_names ={'age', 'sex', 'cp', 'trestbps', 'chol', 'fbs', 'restecg', 'thalach', 'exang', 'oldpeak', 'slope','ca', 'thal', 'target'};%Column labels for the input data
data = array2table(data_set, 'VariableNames', col_names); %Append the column name to the table

%Checking for null and empty spaces in data
checking_data = ismissing(data,{'' '.' 'NA' NaN -99});
rowsWithMissing = data(any(checking_data,2),:);

if ~(isequal(rowsWithMissing, 0))
    disp("Data is already cleaned")
else
    data = rmmissing(data);
    save('cleveland_heart_disease_dataset_labelled.mat', data);
end

%Data visualization with age,disease count and classification type
x1 = unique(data.age); % gives the unique value of the age 
dis_0 = data((data.target == 0),:); %Return the rows of dataset with target value 0
dis_1 = data((data.target == 1),:); %Return the rows of dataset with target value 1
dis_2 = data((data.target == 2),:); %Return the rows of dataset with target value 2
...
for i = 1:4%Loop for getting target count values with respect to all ages
    a=size(data((data.target == 0)&(data.cp == i),:),1);
    b=size(data((data.target == 1)&(data.cp == i),:),1);
    c=size(data((data.target == 2)&(data.cp == i),:),1);
    y2(i,:) = [a, b, c];
end
...
% Potting of corelation plot between features and target
figure(5);
co_efficient = corrcoef(data_set);
h = heatmap(col_names, col_names,co_efficient);
